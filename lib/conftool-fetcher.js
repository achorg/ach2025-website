...
class ConfToolFetcher {
  ...
  async fetchAdminExport(exportSelect, extraParams = {}) {
    ...
    const cachePath = this.getCachePath(`adminExport_${exportSelect}`);
  ...
  async fetchMultiple(requests) {
    const entries = await Promise.all(
      requests.map(async ({ key, exportSelect, extraParams }) => [
        key,
        await this.fetchAdminExport(exportSelect, extraParams)
      ])
    );
