...
<div class="alert alert-info" role="alert" style="background:#fff8e1; border-left:4px solid #f4b400; padding:0.75rem 1rem; margin-bottom:1.25rem; border-radius:3px;">
  <strong>Experimental page.</strong> This is a sandbox for trying out new visualizations and layout changes for the program. The live program lives at <a href="/en/program/">/en/program/</a>. Anything broken here will not affect production.
</div>
...
<div class="viz-section">
  <h3>Author-selected keywords</h3>
  <p class="text-muted">{{ conftool.totalKeywords }} distinct keywords typed by authors across {{ conftool.totalPapers }} papers — sized by how many papers chose each.</p>
  <div class="topic-cloud" aria-label="All author-selected keywords, sized by frequency">
...
<div class="viz-section">
  <h3>ConfTool topics</h3>
  <p class="text-muted">{{ conftool.totalTopics }} controlled-vocabulary topics selected by authors across {{ conftool.totalPapers }} papers — sized by how many papers chose each.</p>
  <div class="topic-cloud topic-cloud--topics" aria-label="ConfTool topics, sized by frequency">
    {% for item in conftool.allTopics %}
    <span class="topic-tag topic-tag--topic" style="font-size:{{ item.sizeRem }}rem" title="{{ item.count }} paper{% if item.count != 1 %}s{% endif %}">{{ item.topic }}</span>
    {% endfor %}
  </div>
</div>
{% elif conftool.totalTopics === 0 %}
<div class="viz-section">
  <p class="text-muted"><em>No ConfTool topics returned by the API for this build. (Either topics aren't enabled in the ConfTool admin, or the field name is one we didn't probe — see the build log for the field-name diagnostic.)</em></p>
</div>
{% endif %}
        ...
        <details class="prog-papers" open>
          ...
          <ul class="prog-paper-list">
            ...
            <li class="prog-paper">
              ...
              {% endif %}
              {% if paper.topics and paper.topics.length > 0 %}
              <div class="prog-paper-kws prog-paper-topics">
                {% for t in paper.topics %}<span class="prog-kw prog-kw--topic">{{ t }}</span>{% endfor %}
              </div>
              {% endif %}
            </li>
            {% endfor %}
...
<style>
  ...
  .topic-tag--topic:hover { background: #c8e9d3; }
  .prog-kw--topic {
    background: #e1f3e7;
    color: #1b6c3a;
  }
  .prog-paper-topics { margin-top: 0.15rem; }
